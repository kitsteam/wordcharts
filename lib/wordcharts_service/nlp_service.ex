defmodule WordchartsService.NlpService do
  ...
  @verb_classes ["VV(FIN)", "VA(FIN)","VM(FIN)", "VV(IMP)", "VA(IMP)", "VV(INF)", "VV(IZU)", "VA(INF)", "VM(INF)", "VV(PP)", "VM(PP)", "VA(PP)",
                 "VB", "VBD", "VBG", "VBN", "VBZ", "HV", "HVD", "HVG", "HVN", "HVZ", "BE", "BED", "BEDZ", "BEG", "BEM", "BEN", "BER", "BEZ"]
  @adj_classes ["ADJ(D)", "ADJ(A)",
                "JJ", "JJR", "JJS", "JJT"]
  @article_classes ["ART",
                    "AT"]
  @con_classes ["KOUS", "KOUI", "KON", "KOKOM",
                "CC", "CS", "DTX"]
  @prep_classes ["APPR", "APPO", "APZR", "APPRART",
                 "IN"]
  @adverb_classes ["ADV",
                   "B", "RBR", "RBT", "RN", "RP"]
  @pron_classes ["PPER", "PRF", "PPOSAT", "PPOSS", "PDAT", "PDS", "PIAT", "PIS", "PRELAT", "PWAT", "PWS", "PWAV", "PROAV",
  ...
  def tag_words(word_string, lang \\ "en", config \\ default_config()) do
    ...
    |> Enum.reject(fn match ->
      Enum.member?(@ignore_classes, match["wordClass"]) || String.trim(match["wordClass"]) == "" || Enum.member?(@ignore_chars, match["original"])
    end)
    |> Enum.map(fn match ->
      %{
        "name" => match["original"],
